...
function getAuthenticatedUser() {
  ...
  if (result.status !== 0) throw new Error(`Failed to get authenticated user: ${result.stderr}`);
...
function ghGraphql(query, variables) {
  ...
  if (result.status !== 0) {
    throw new Error(`GitHub GraphQL call failed: ${[result.stderr, result.stdout].filter(Boolean).join('\n').trim()}`);
  }
