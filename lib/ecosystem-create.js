...
function createEcosystem(args) {
  ...
  const ecosystemDir = path.join(ROOT, 'ecosystems', directoryName);
  if (fs.existsSync(ecosystemDir)) {
    throw new Error(`Ecosystem "${directoryName}" already exists: ${ecosystemDir}`);
  }
  ...
  const sddDir = path.join(ecosystemDir, sddRoot);
  const tasksDir = path.join(sddDir, 'tasks');
  const skillsDir = path.join(ecosystemDir, 'skills');
  const runsDir = path.join(ecosystemDir, historyRoot);
  ...
  const config = {
    ...
    repositories: repositories.map((repo) => ({
      id: repo.id,
      label: repo.label,
      path: formatRepositoryConfigPath(ecosystemDir, repo.path),
...
function resolveGitHubProjectDecision(args) {
  ...
  throw new Error(
    'GitHub Project URL is required unless skipGithubProject is true. ' +
      'Ask the user for the GitHub Project URL, or ask them to confirm that this ecosystem does not need a Project.',
  );
}
...
function formatRepositoryConfigPath(ecosystemDir, repositoryPath) {
  const relativePath = path.relative(ecosystemDir, repositoryPath) || '.';
...
function buildSddReadme({ name, githubProject, repositories, docsQualityBaseline }) {
  return [
    `# ${name} SDD`,
    '',
    `This ecosystem centralizes execution planning for ${name} while keeping code and human documentation inside their owning repositories.`,
    '',
    ...(githubProject ? [`GitHub Project: ${githubProject.url}`, ''] : ['GitHub Project: not configured', '']),
    '## Structure',
    '',
    '- `tasks/`: executable ecosystem tasks for this ecosystem queue',
    '- use the shared `ecosystem-task-factory` skill to create tasks later, intentionally',
    '',
    'This bootstrap does not pre-create implementation tasks. Future work for this ecosystem should be created under this ecosystem `sdd/tasks/` directory through `ecosystem-task-factory`.',
    '',
    '## Repositories',
    '',
...
function formatDocsQualityBaseline(value) {
  if (!Array.isArray(value) || value.length === 0) {
    return [
      '- not recorded by `create_ecosystem`',
