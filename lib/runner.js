...
function runWithRunner(args, getEcosystem) {
  ...
  const boardSync = [];
  if (!dryRun && selection === 'task') {
    boardSync.push(setTaskBoardStatus({ ecosystem: ecosystem.directoryName, task: args.value, status: 'in-progress' }, () => ecosystem));
  }

  const result = spawnSync('npm', commandArgs, { cwd: ROOT, encoding: 'utf8', maxBuffer: 1024 * 1024 * 10 });

  if (!dryRun && selection === 'task' && result.status === 0) {
    boardSync.push(setTaskBoardStatus({ ecosystem: ecosystem.directoryName, task: args.value, status: 'testing' }, () => ecosystem));
  }
