...
function createGitHubProject({ ecosystem, title }) {
  const ownerId = getOwnerId(ecosystem.githubProject || inferOwner(ecosystem));
  const ownerInfo = ecosystem.githubProject || inferOwner(ecosystem);
  ...
  updateEcosystemConfig(ecosystem.configPath, githubProject);
...
function inferOwner(ecosystem) {
  // Try to infer from first repository's git remote
  const repo = ecosystem.repositories[0];
  if (!repo) throw new Error('Ecosystem has no repositories to infer GitHub owner from.');

  const result = spawnSync('git', ['-C', repo.root, 'config', '--get', 'remote.origin.url'], { encoding: 'utf8' });
  if (result.status !== 0) throw new Error('Cannot infer GitHub owner: no git remote found.');
...
function updateEcosystemConfig(configPath, githubProject) {
...
function ghGraphql(query, variables) {
  ...
  if (result.status !== 0) {
    throw new Error(`GitHub GraphQL failed: ${[result.stderr, result.stdout].filter(Boolean).join('\n').trim()}`);
  }
