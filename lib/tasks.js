...
function setTaskStatus(args, getWorkspace) {
  ...
  if (status === 'done') {
    updateTaskFrontmatter(task.filePath, { finishedAt: new Date().toISOString() });
  }
  ...
  return { workspace: ws.directoryName, task: updatedTask, readme, githubSync, ...(createdPRs ? { createdPRs } : {}) };
}
...
function createBranchInRepos(ws, repoIds, branchName, baseBranch) {
  ...
  for (const repoId of repoIds) {
    ...
    if (checkout.status !== 0) {
      results.push({ repository: repoId, success: false, error: `checkout ${baseBranch} failed: ${(checkout.stderr || '').trim()}` });
      continue;
    }
    ...
    if (create.status !== 0) {
      if ((create.stderr || '').includes('already exists')) {
        ...
        results.push({ repository: repoId, success: false, error: (create.stderr || '').trim() });
      }
      continue;
    }
