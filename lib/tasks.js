...
function setTaskStatus(args, getEcosystem) {
  ...
  if (status === 'implemented') {
    const board = syncBoardForStatus(ecosystem, task, BOARD_STATUS.testing);
...
function startTaskExecution(args, getEcosystem) {
  const ecosystem = getEcosystem(args.ecosystem);
  const task = resolveTask(ecosystem, args.task);
  const board = syncBoardForStatus(ecosystem, task, BOARD_STATUS.inProgress);
  if (board.enabled) {
    updateTaskFrontmatter(task.filePath, { githubProjectStatus: board.status.name });
  }
  const updatedTask = parseTaskFile(task.filePath);
  rememberActiveTasks(ecosystem.directoryName, [updatedTask.id], 'execution-started');
  return { ecosystem: ecosystem.directoryName, task: updatedTask, githubSync: board };
}

function finishTaskExecution(args, getEcosystem) {
  const ecosystem = getEcosystem(args.ecosystem);
  const task = resolveTask(ecosystem, args.task);
  const board = syncBoardForStatus(ecosystem, task, BOARD_STATUS.testing);
  if (board.enabled) {
  ...
  const updatedTask = parseTaskFile(task.filePath);
  rememberActiveTasks(ecosystem.directoryName, [updatedTask.id], 'execution-finished');
  return { ecosystem: ecosystem.directoryName, task: updatedTask, githubSync: board };
}
...
function setTaskBoardStatus(args, getEcosystem) {
  ...
  const board = syncBoardForStatus(ecosystem, task, statusName);
  if (board.enabled) {
    updateTaskFrontmatter(task.filePath, { githubProjectStatus: board.status.name });
  }
  const updatedTask = parseTaskFile(task.filePath);
...
function syncBoardForStatus(ecosystem, task, statusName) {
  ...
  return moveTaskCard(ecosystem, task, statusName);
}
