...
function createScopeBranch(args, getWorkspace) {
  const ws = getWorkspace(args.workspace);
  const branchBase = args.baseBranch || 'main';

  // Resolve affected repos from scope or taskIds
  const tasks = resolveBranchTasks(ws, args);
  const repoIds = [...new Set(tasks.flatMap((t) => t.repositories))];

  if (repoIds.length === 0) throw new Error('No repositories resolved for the given scope/tasks.');

  // Derive branch name: explicit, or from scope, or from first task id
  const branchName = args.branchName || deriveBranchName(tasks, args.scope);

  const results = [];
  for (const repoId of repoIds) {
    const repo = ws.repositories.find((r) => r.id === repoId);
    if (!repo) throw new Error(`Repository "${repoId}" not found in workspace.`);

    // Ensure we're on baseBranch first, then create new branch
    const checkout = spawnSync('git', ['checkout', branchBase], { cwd: repo.root, encoding: 'utf8' });
    if (checkout.status !== 0) {
      results.push({ repository: repoId, success: false, error: `Failed to checkout ${branchBase}: ${(checkout.stderr || '').trim()}` });
      continue;
    }
    ...
    if (create.status !== 0) {
      ...
      if (stderr.includes('already exists')) {
        const switchResult = spawnSync('git', ['checkout', branchName], { cwd: repo.root, encoding: 'utf8' });
        results.push({ repository: repoId, success: switchResult.status === 0, branch: branchName, alreadyExisted: true, error: switchResult.status !== 0 ? (switchResult.stderr || '').trim() : null });
      } else {
  ...
  return { workspace: ws.directoryName, branch: branchName, baseBranch: branchBase, repositories: results, allSuccess, tasks: tasks.map((t) => t.id) };
}

function resolveBranchTasks(ws, args) {
  if (args.scope) {
    const tasks = listTasks(ws, { scope: args.scope });
    if (tasks.length === 0) throw new Error(`Scope "${args.scope}" did not match any tasks.`);
    return tasks;
  }
  if (args.taskIds && args.taskIds.length > 0) {
    return args.taskIds.map((id) => resolveTask(ws, id));
  }
  throw new Error('Provide either scope or taskIds to determine affected repositories.');
}
