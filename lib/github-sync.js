...
function createTaskCard(ecosystem, task) {
  ...
  const issue = ecosystem.githubProject.issueAssignee
    ? convertDraftProjectItemToIssue(
        draft.projectItem.id,
        primaryRepository.githubFullName,
        ecosystem.githubProject.issueAssignee,
      )
    : null;

  return {
    enabled: true,
    draftIssue: issue ? null : {
...
function convertDraftProjectItemToIssue(itemId, githubFullName, assignee) {
  const [owner, name] = githubFullName.split('/');
  const repository = ghGraphql(
    `query($owner: String!, $name: String!) {
      repository(owner: $owner, name: $name) { id }
    }`,
    { owner, name },
  ).repository;
  if (!repository) throw new Error(`GitHub repository not found: ${githubFullName}`);

  const converted = ghGraphql(
    ...
    { itemId, repositoryId: repository.id },
  ).convertProjectV2DraftIssueItemToIssue.item.content;

  const updated = ghApi(
    'PATCH',
    `/repos/${githubFullName}/issues/${converted.number}`,
    { assignees: [assignee] },
  );
  return {
    repo: githubFullName,
...
function updateTaskCardCloseout(ecosystem, task, closeout) {
  ...
  if (task.githubIssueRepo && task.githubIssueNumber) {
    const updated = ghApi(
      'PATCH',
      `/repos/${task.githubIssueRepo}/issues/${task.githubIssueNumber}`,
      { body },
    );
    return {
      enabled: true,
  ...
  if (!task.githubDraftIssueNodeId) {
    throw new Error(
      `Task "${task.id}" does not have GitHub issue or draft issue frontmatter.`,
    );
  }
