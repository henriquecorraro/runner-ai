...
function rollbackTaskCreation({ ws, projectItem, issues, cause }) {
  const cleanupErrors = cleanupTaskCreationArtifacts({ ws, projectItem, issues, cause });
...
function cleanupTaskCreationArtifacts({ ws, projectItem, issues, cause }) {
  const cleanupErrors = [];
...
function rollbackCreatedTaskCard(ws, githubSync, cause) {
  if (!githubSync || !githubSync.enabled) return { attempted: false, cleanupErrors: [] };
  const cleanupErrors = cleanupTaskCreationArtifacts({
    ws,
    projectItem: githubSync.projectItem && githubSync.projectItem.nodeId
      ? { id: githubSync.projectItem.nodeId }
      : null,
    issues: githubSync.issues || [],
    cause,
  });
  return { attempted: true, cleanupErrors };
}
...
function inferGitHubFullName(repositoryRoot) {
  ...
  if (result.status !== 0) {
    throw new Error(`Could not read git remote origin for repository root: ${repositoryRoot}${result.error ? `: ${result.error.message}` : ''}`);
  }
