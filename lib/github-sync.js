...
function createTaskCard(ecosystem, task) {
  ...
  const plan = prepareTaskCreationPlan(ecosystem, task);
  const issues = [];
  let projectItem = null;

  try {
    const body = buildTaskCardBody(ecosystem, task, plan.primaryRepository);
    ...
    const status = updateProjectItemStatusByResolvedOption(ecosystem.githubProject, itemDatabaseId, plan.todoStatus);
    const projectUrl = projectItemUrl(ecosystem.githubProject, itemDatabaseId);
    const linkedBody = buildTaskCardBody(ecosystem, task, plan.primaryRepository, null, {
      issues,
    ...
    rollbackTaskCreation({ ecosystem, projectItem, issues, cause: error });
  }
}
...
function resolvePrimaryRepository(ecosystem, task) {
  return resolveTaskRepositories(ecosystem, task)[0];
}

function resolveTaskRepositories(ecosystem, task) {
  return task.repositories.map((repositoryId) => {
    const repository = ecosystem.repositories.find((repo) => repo.id === repositoryId);
...
function prepareTaskCreationPlan(ecosystem, task) {
  const repositories = resolveTaskRepositories(ecosystem, task);
  ...
  const projectId = getProjectNodeId(ecosystem.githubProject);
  const todoStatus = resolveProjectStatusOption(ecosystem.githubProject, BOARD_STATUS.todo);
...
function rollbackTaskCreation({ ecosystem, projectItem, issues, cause }) {
  const cleanupErrors = [];

  if (projectItem && projectItem.id) {
    try {
      deleteProjectItem(getProjectNodeId(ecosystem.githubProject), projectItem.id);
    } catch (error) {
...
function buildRollbackComment(cause) {
  return [
    'ecosystem-ai-runner could not complete deterministic task setup.',
    '',
    `Failure: ${cause.message}`,
    '',
    'This issue was closed automatically because the local task was not recorded.',
    '',
    '<!-- ecosystem-ai-runner:rollback -->',
  ].join('\n');
}
...
function buildTaskCardBody(ecosystem, task, primaryRepository, closeout = null, links = {}) {
